...
def cnn_map(x_):
    xx = tf.layers.batch_normalization(x_)#batch normalize
    ...
    shape1 = pool_flat.shape[1]
    W1 = tf.get_variable('W', shape=[shape1, 500])
    b1 = tf.get_variable('b', shape=[500])
    h_fc = tf.nn.relu(tf.matmul(pool_flat, W1) + b1)
    dropout = tf.layers.dropout(h_fc, 0.5)
...
def cnn_expanded(x_):
    ...
    pool_flat = tf.contrib.layers.flatten(pool4, scope='poolflat')
    shape1 = pool_flat.shape[1]
    W1 = tf.get_variable('W', shape=[shape1 , 800])
    b1 = tf.get_variable('b', shape=[800])
    h_fc = tf.nn.relu(tf.matmul(pool_flat, W1) + b1)
    dropout = tf.layers.dropout(h_fc, 0.5)
    dense = tf.layers.dense(inputs=dropout, units=400, activation=tf.nn.relu)
    logits = tf.layers.dense(inputs=dense, units=10)
    return logits
