...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    if self.activation is 'relu':
        ...
        too_big = np.argwhere (z1>100).T
        if (len(too_big) == 0):
            hidden = np.log(1+np.exp(z1))
        else:
            not_big = np.argwhere(z1<=100).T
            hidden = np.zeros(np.shape(z1))
            hidden[not_big[0],not_big[1]] = np.log(1+np.exp(z1[not_big[0],not_big[1]]))
            hidden[too_big[0],too_big[1]] = z1[too_big[0], too_big[1]]
            
    elif self.activation is 'sigmoid':
            hidden = 1/(1+np.exp(-z1.clip(-100, None)))            
    else:
    ...
    ###########################################################################
    #log_sum_exp
    scores_star = np.nanmax(scores, axis=1, keepdims=True)
    exp_scores = np.exp(scores - scores_star)
    probs = (scores - (np.log(np.sum(exp_scores, axis=1, keepdims=True)) + scores_star))[range(N),y]
    data_loss = - 1/N * np.sum(probs)
    reg_loss = reg * (np.sum(W1*W1) + np.sum(W2*W2))
    lamb = 0.5
    loss = data_loss + lamb * reg_loss
    ...
    dscore = np.exp(scores)/np.sum(np.exp(scores), axis=1, keepdims=True)
    #dscore = probs
    dscore[range(N),y] -= 1
    dscore /= N
    
    dW2 = hidden.T.dot(dscore) + reg*W2    # partial derivative of loss wrt. W2
    db2 = np.ones(N).dot(dscore)     # and so on...
    ...
    if self.activation is 'relu':
        ...
        too_big = np.argwhere (z1>100).T
        if (len(too_big) == 0):
            dz1 = dhidden * (np.exp(z1)/(1+np.exp(z1)))
        else:
            not_big = np.argwhere(z1<=100).T
            dz1 = np.zeros(np.shape(z1))      
            dz1[not_big[0],not_big[1]] = dhidden[not_big[0],not_big[1]] * (np.exp(z1[not_big[0],not_big[1]]))/(1 + np.exp(z1[not_big[0],not_big[1]]))
            dz1[too_big[0],too_big[1]] = dhidden[too_big[0], too_big[1]]
